#Same as TM.py but integrated with discord.py|2.0 and the new slash commands and other features
import discord
...
bot = commands.Bot(command_prefix='TM!',intents=intents,status=discord.Status.dnd,activity=discord.Activity(name='/setup',type=discord.ActivityType.watching))
...
@bot.command()
@commands.is_owner()
async def DMstar(ctx,user:discord.User):
    ...
    if ctx.author.id == 705992469426339841:
        ...
        for guilds in bot.guilds:
            for members in guilds.members:
                DM_alr = False
                person = bot.get_user(members.id)
                print(person)
                if person.id == bot.user.id:
                    continue
                if person.bot == False:
                    peep +=1
                    async for message in person.history(limit=1):
                        DM_alr = True
                        if message:
                            count +=1
                            break
                    ...
                    print(person.bot)
                    count+=1
        await ctx.send(f"Done send {counter} skipped {count} peeps {peep}")
...
async def setup(ctx:discord.Interaction,game:str):
    ...
    if perp != None:
        ...
        if x != None:
            await ctx.followup.send('Error: You have already set up a profile. To add games to your profile use the addgame command')
            return
        else:
...
@commands.guild_only()
async def search(ctx:discord.InteractionResponse,game:str):
    ...
    if found != None:
        ...
        if z == None:
            ...
            if game == 'Rocket League':
                if x == None:
                    await ctx.followup.send('Error you dont have Rocket League added to your profile. Please do `/addgame` to do that.')
                    return
                ...
                if not y:
                    info = {'game':'RL','user': ctx.user.id, 'rank': f"{x.get('rank')}",'region':f"{z.get('region')}", 'time': 0}
                ...
                if rbx.find_one({'user':ctx.user.id}) == None:
                    await ctx.followup.send('Error: You havnt added this game to your profile. Please do that with `/addgame`.')
                else:
...
@search.error
async def search_error(ctx:discord.InteractionResponse,error):
    if isinstance(error,commands.MaxConcurrencyReached):
        await ctx.followup.send('Error: You already have a search running. If you are trying to cancel your search please do the `/cancel` command.')
...
@bot.tree.command(guild=discord.Object(877460893439512627))
@app_commands.choices(game=supported_games)
async def addgame(ctx:discord.InteractionResponse,game:str):#"Rocket League", "Roblox", "Minecraft", "Valorant", "Fortnite"
...
@bot.tree.command(guild=discord.Object(877460893439512627))
@app_commands.choices(game=supported_games)
async def removegame(ctx:discord.InteractionResponse,game:str):
    """Remove games from your personal profile."""
    await ctx.reponse.defer()
    if game == 'Rocket League':
...
async def blacklist(ctx:discord.Interaction,function:str,*,reason:str = None):
...
@blacklist.error
async def bl_error(ctx:discord.InteractionResponse,error):
...
@bot.tree.command(guild=discord.Object(877460893439512627),description='Delete your profile.')
...
async def invite(ctx):
    ...
    embed = discord.Embed(title='Invite TM to your server today!',description='[click here](https://discord.com/api/oauth2/authorize?client_id=822637954769879100&permissions=414531791937&scope=bot%20applications.commands)',color=0xCC071F)
...
@bot.tree.command(guild=discord.Object(877460893439512627),name='help')
async def help(ctx):
    """The help section"""
...
@bot.tree.command(guild=discord.Object(877460893439512627),name='vote')
...
@bot.tree.command(guild=discord.Object(877460893439512627),name='panel')
