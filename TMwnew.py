...
RocketLeague_game = app_commands.Choice(name='Rocket League', value='Rocket League')
Valorant_game = app_commands.Choice(name='Valorant', value='Valorant')
Fortnite_game = app_commands.Choice(name='Fortnite', value='Fortnite')
supported_games.append(Roblox_game)
supported_games.append(RocketLeague_game)
supported_games.append(Valorant_game)
supported_games.append(Fortnite_game)
intents = discord.Intents.all()

bot = commands.Bot(command_prefix='TM!', intents=intents, status=discord.Status.dnd,
                   activity=discord.Activity(name='/setup', type=discord.ActivityType.watching))
...
async def on_command_error(ctx, error):
    if isinstance(error, commands.BotMissingPermissions):
        await ctx.send('Error: I am missing the permissions to operate properly. Please fix my permissions.')
    elif isinstance(error, commands.NoPrivateMessage):
        embed = discord.Embed(title='While running, an error occured.',
                              description='This command doesnt work in dm\'s please try it in a guild.', color=0xCC071F)
        await ctx.send(embed=embed)
...
# Option(str,"The game to setup with",required=True,choices=supported_games)
# region:Option(str,"Your closest location. For games that rely on ping." ,required=True,choices=region)
async def setup(ctx: discord.Interaction, game: str):
    ...
    if perp != None:
        embed = discord.Embed(title=f'Error: You have been blacklisted from our services for: {perp.get("reason")} ',
                              description='If You wish to appeal your ban than please join our support server [here](https://discord.gg/5XAubY2v3N) and open a ticket.',
                              color=0xCC071F)
        await ctx.followup.send(embed=embed)
    else:
        if x != None:
            await ctx.followup.send(
                'Error: You have already set up a profile. To add games to your profile use the addgame command')
            return
        else:
            if game == 'Rocket League':
                ...
                async def button_callback(interaction: discord.Interaction):
                    if interaction.user.id == ctx.user.id:
                        if interaction.data.get('custom_id') != 'Cancel':
                            ...
                            await UserProfiles.location(ctx.user, ctx.channel_id)
                ...
                try:
                    ...
                    await ctx.followup.send('You didnt respond... Cancelling setup.')
                else:
                    await UserProfiles.location(ctx.user, ctx.channel_id)
                    rbx.insert_one({'user': ctx.user.id, 'name': f'{answer.content}'})
            ...
            #     def check(message):
            #         return (message.content in platform) and (message.channel == ctx.channel) and (message.author == ctx.user)
            #     try:
            ...
            #             def check(message):
            #                 return (message.content in gamemode) and (message.channel == ctx.channel) and (message.author == ctx.user)
            #             try:
            ...
            #                                           color=0xCC071F)
            #                     MC.insert_one({'user':ctx.user.id,'platform':'bedrock','mode':f'{mode}','IGN':f'{IGN}'})
            #                     await ctx.followup.send(embed = embed)
            elif game == 'Valorant':
                ...
                async def button_callback(interaction: discord.Interaction):
                    if interaction.user.id == ctx.user.id:
                        if interaction.data.get('custom_id') != 'Cancel':
                            ...
                            await interaction.response.edit_message(embed=embed2, view=None)
                            await UserProfiles.location(ctx.user, ctx.channel_id)
                            VAL = {'user': ctx.user.id, 'rank': f'{rank}'}
                            val.insert_one(VAL)
                        else:
                            await interaction.response.edit_message(content="Cancelled", embed=None, view=None)
                ...
                async def button_callback(interaction: discord.Interaction):
                    if interaction.user.id == ctx.user.id:
                        if interaction.data.get('custom_id') != 'Cancel':
                            ...
                            await interaction.response.edit_message(embed=embed2, view=None)
                            await UserProfiles.location(ctx.user, ctx.channel_id)
                            forts = {'user': ctx.user.id, 'rank': f'{rank}'}
...
async def search(ctx: discord.Interaction, game: str):
    ...
    if found != None:
        ...
        info = {'user': ctx.user.id}
        x = RL.find_one(info)
        z = profiling.find_one(info)
        if z == None:
            await ctx.followup.send('Error you didn\'t set up your profile. Please do `/setup` to do that.')
        else:
            if game == 'Rocket League':
                if x == None:
                    await ctx.followup.send(
                        'Error you dont have Rocket League added to your profile. Please do `/addgame` to do that.')
                    return
                embed = discord.Embed(title='Now Searching for teammates for the game: Rocket League',
                ...
                if rbx.find_one({'user': ctx.user.id}) == None:
                    await ctx.followup.send(
                        'Error: You havnt added this game to your profile. Please do that with `/addgame`.')
                else:
                    ...
                    if not y:
                        ...
                        if mel == False:
                            ...
                            match.insert_one(info)
            # elif game == 'Minecraft':
            #     if MC.find_one({'user':ctx.user.id}) == None:
            #         await ctx.followup.send('Error you have not setup this game in your profile. Please do so with `/addgame`.')
            #     else:
            #         embed = discord.Embed(title='Now Searching for teammates for the game: Minecraft',
            #                           description='This lasts an hour at max, after an hour with no teammate found you will be DM\'ed to try again.',
            #                           color=0xCC071F)
            #         await ctx.followup.send(embed = embed)
            #         yz = MC.find_one({'user':ctx.user.id})
            ...
            #             if yz.get('stars') != None:
            #                 star = Profiles.MC_ranks(yz.get('stars'))
            #                 info = {'user': ctx.user.id,'game': 'MC', 'region': f"{z.get('region')}", 'rank': f'{star}',
            #                         'platform': f'{yz.get("platform")}','time':0}
            #             else:
            #                 info = {'user': ctx.user.id,'game': 'MC', 'region': f"{z.get('region')}",
            #                         'platform': f'{yz.get("platform")}', 'mode': f'{yz.get("mode")}','time':0}
            #             match.insert_one(info)
            #         if user == None:
            #             return
            #         else:
            #             mel = await UserProfiles.teammateyes(ctx:discord.InteractionResponse, user)
            #             if mel == False:
            ...
            #                 match.insert_one(info)
                ...
                if val.find_one({'user': ctx.user.id}) == None:
                    await ctx.followup.send(
                        'Error you have not setup this game in your profile. Please do so with `/addgame`.')
                else:
                ...
                if fort.find_one({'user': ctx.user.id}) == None:
                    await ctx.followup.send(
                        'Error you have not setup this game in your profile. Please do so with `/addgame`.')
                else:
...
@search.error
async def search_error(ctx: discord.Interaction, error):
    if isinstance(error, commands.MaxConcurrencyReached):
        await ctx.followup.send(
            'Error: You already have a search running. If you are trying to cancel your search please do the `/cancel` command.')
...
@bot.tree.command()
async def cancel(ctx: discord.Interaction):
    """cancel your search"""
    await ctx.response.defer()
    x = match.find_one({"user": ctx.user.id})
    if x == None:
        await ctx.followup.send('You dont have a current running search to cancel.')
    else:
            ...
            if ans != 'No':
                try:
                    ...
                    await interaction.response.edit_message(embed=None, view=None,
                                                            content="Successfully removed from the queue.")
                except:
                    await interaction.response.edit_message(embed=None, view=None,
                                                            content="Having trouble removing from queue. Please try again.")
            else:
...
@bot.tree.command()
@app_commands.choices(game=supported_games)
async def addgame(ctx: discord.Interaction,game: str):  # "Rocket League", "Roblox", "Minecraft", "Valorant", "Fortnite"
    ...
    if x == None:
        ...
        if game == 'Rocket League':
            ...
            x = rbx.find_one({'user': ctx.user.id})
            if x != None:
                await ctx.followup.send(
                ...
                try:
                    ...
                    rbx.insert_one({'user': ctx.user.id, 'name': f'{answer.content}'})
        # elif game == 'Minecraft':
        #     x = MC.find_one({'user':ctx.user.id})
        #     if x != None:
        #         await ctx.followup.send('Error You already have this game on your profile. If you need to remove the game try the `TM!remove` command.')
        #     else:
        #         embed = discord.Embed(title='Do you play Minecraft Java or Bedrock?', color=0xCC071F)
        #         await ctx.followup.send(embed=embed)
        #         platform = ['Java', 'java', 'Bedrock', 'bedrock']
        ...
        #                                                       description='If you want to find a partner right away then try my `TM!search` command.',
        #                                                       color=0xCC071F)
        #                                 info = {'user': ctx.user.id, 'platform': f'{platform}', 'mode': 'pvp',
        ...
        #                                                   color=0xCC071F)
        #
        #                             info = {'user': ctx.user.id, 'platform': f'{platform}', 'mode': 'survival',
        #                                     'IGN': f'{IGN}'}
        #                             MC.insert_one(info)
        #                             await ctx.followup.send(embed=embed)
        #
        ...
        #                                               description='If you want to find a partner right away then try my `TM!search` command.',
        #                                               color=0xCC071F)
        #                         MC.insert_one({'user': ctx.user.id, 'platform': 'bedrock', 'mode': f'{mode}',
        #                                        'IGN': f'{IGN}'})
        #                         await ctx.followup.send(embed=embed)
        #                         return
        elif game == 'Valorant':
...
@bot.tree.command()
@app_commands.choices(game=supported_games)
async def removegame(ctx: discord.Interaction, game: str):
    ...
    if game == 'Rocket League':
        ...
        async def button_callbacks(interaction: discord.Interaction):
            if interaction.data.get('custom_id') != 'Cancel':
                try:
                    RL.delete_one({'user': ctx.user.id})
                    await interaction.response.edit_message(embed=None, view=None,
        ...
        async def button_callbacks(interaction: discord.Interaction):
            if interaction.data.get('custom_id') != 'Cancel':
                try:
                    rbx.delete_one({'user': ctx.user.id})
                    await interaction.response.edit_message(embed=None, view=None,
                                                            content="Successfully removed the game.")
                except:
                    await interaction.response.edit_message(embed=None, view=None,
    ...
    #         if interaction.data.get('custom_id') != 'Cancel':
    #             try:
    #                 MC.delete_one({'user': ctx.user.id})
    #                 await interaction.response.edit_message(embed=None, view=None,
    #                                                         content="Successfully removed the game.")
    #             except:
    #                 await interaction.response.edit_message(embed=None, view=None,
    ...
    #     await ctx.followup.send(embed=embed, view=view)
        ...
        async def button_callbacks(interaction: discord.Interaction):
            if interaction.data.get('custom_id') != 'Cancel':
                try:
                    fort.delete_one({'user': ctx.user.id})
                    await interaction.response.edit_message(embed=None, view=None,
                                                            content="Successfully removed the game.")
                except:
                    await interaction.response.edit_message(embed=None, view=None,
...
@bot.tree.command()
async def report(ctx: discord.Interaction):
    """report users."""
    await ctx.response.defer()
    embed = discord.Embed(title='Thank you for using our report feature to try and help keep our community troll-free',
                          description='Now when submitting your report please include all of the following... Who are you reporting. Why are you reporting(include name and # or just user id. Your choice.). Add any sort of evidence.(pictures are accepted).',
                          color=0xCC071F)
    await ctx.followup.send(embed=embed)
    ...
    try:
        ...
        embed = discord.Embed(title='You had 10 minutes to submit your report.',
                              description='If that isnt enought time just join our support server to report the user which you can find [here](https://discord.gg/5XAubY2v3N)',
                              color=0xCC071F)
        await ctx.followup.send(embed=embed)
    else:
        embed = discord.Embed(title='Thank you for submitting a report.',
                              description='Our Moderators will take a look at it soon.', color=0xCC071F)
        await ctx.followup.send(embed=embed)
        embed = discord.Embed(title=f'New report Submitted by {ctx.user}', description=f'{x.content}', color=0xCC071F)
        if x.attachments:
...
async def getid(ctx, user: discord.User):
    if user != None:
        await ctx.followup.send('Found')
        await ctx.followup.send(user.id)
    else:
        await ctx.followup.send('Couldnt find user...')


BL_function = []
BL_add = app_commands.Choice(name='add', value='add')
BL_remove = app_commands.Choice(name='remove', value='remove')
BL_find = app_commands.Choice(name='find', value='find')
BL_function.append(BL_find)
BL_function.append(BL_add)
BL_function.append(BL_remove)


@bot.tree.command(guild=discord.Object(877460893439512627), name="blacklist", description='Admin Blacklisting Users')
@app_commands.choices(function=BL_function)
async def blacklist(ctx: discord.Interaction,user:discord.User, function: str, *, reason: str = None):
    if ctx.user.id == 705992469426339841:
        if function == 'add':
            if reason == None:
                await ctx.followup.send("please add a reason.")
                return
            await ctx.followup.send(f'Added {user}. Reason: {reason}')
            BL.insert_one({'user': user.id, 'reason': f'{reason}'})
        elif function == 'remove':
            BL.delete_one({'user': user.id})
            await ctx.followup.send(f'Removed {user}')
        elif function == 'find':
            x = BL.find_one({'user': user.id})
            if x == None:
                await ctx.followup.send("User not found in blacklist")
                return
            await ctx.followup.send(f'Blacklist for {user}: {x.get("reason")}')
    else:
        return


@blacklist.error
async def bl_error(ctx: discord.Interaction, error):
    await ctx.response.send_message(error)


@bot.tree.command(guild=discord.Object(877460893439512627), description='Admin deleting profiles')
async def admindelete(ctx: discord.Interaction, user: discord.User):
    if ctx.user.id == 705992469426339841:
        await ctx.response.defer()
        user = user.id
        Profiles.deletion(user)
        await ctx.followup.send('Deleted User\'s profile successfully')
...
async def update():
    ...
    for info in x:
        ...
        if time >= 60:
            embed = discord.Embed(title='It has been an hour and no one has been matched with you.',
                                  description='You could either do the following: Requeue using `/search` or just wait for a little bit of time for some other users who are the same rank as you to queue aswell: sorry for the inconvience',
                                  color=0xCC071F)
...
@bot.tree.command(description='Delete your profile.')
async def delprofile(inter: discord.Interaction):
    await inter.response.defer()

    async def button_callback(interaction: discord.Interaction):
        if inter.user.id == interaction.user.id:
            if interaction.data.get('custom_id') == 'Delete':
                Profiles.deletion(inter.user.id)
    ...
    embed = discord.Embed(title='Are you sure you want to delete your profile.',
                          description='If you say yes you lose all the games set up with your account and you will have to reset it up.',
                          color=0xCC071F)
...
@bot.tree.command()
async def profile(ctx: discord.Interaction, user: discord.User = None):
    ...
    if x == None:
        if user.id != ctx.user.id:
            await ctx.followup.send('Error: User doesnt have a profile.')
        else:
            await ctx.followup.send('Error: user doesnt have a profile. Set up one using the `/setup` command')
    else:
...
async def invite(ctx):
    ...
    embed = discord.Embed(title='Invite TM to your server today!',
                          description='[click here](https://discord.com/api/oauth2/authorize?client_id=822637954769879100&permissions=414531791937&scope=bot%20applications.commands)',
                          color=0xCC071F)
...
async def tutorial(ctx):
    ...
    embed = discord.Embed(title='Do you find TM a bit to hard to understand?',
                          description='Then have a look at a very simple tutorial on how to setup your profile. You can find that video [here](https://youtu.be/dOZHazMxcag)',
                          color=0xCC071F)
    await ctx.response.send_message(embed=embed)
...
async def help(ctx):
    """The help section"""
    embed = discord.Embed(title='Tilted Matchmaking Help Section',
                          description='All of TM\'s commands. If you need any assistance join our help server which you can find [here](https://discord.gg/rKWxkrCkUQ)',
                          color=0xCC071F)
...
async def vote(ctx: discord.Interaction):
    """Vote for us!!!"""
    embed = discord.Embed(title='Vote for us on top.gg!',
                          description='You can find the link [here.](https://top.gg/bot/822637954769879100/vote)',
                          color=0xCC071F)
    await ctx.response.send_message(embed=embed)
